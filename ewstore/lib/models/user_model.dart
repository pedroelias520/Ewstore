...
class UserModel extends Model {
  ...
  void  GetLevelUser(@required int userData) async {
    ...
    for (int i = 0; i < 31; i++) {
      if (userData > levels["Level ${i}"] &&
        ...
        levelCurrentUser["User points"] = userData;
        levelCurrentUser["Total level Points"] =
            levelCurrentUser["Level ${i+1}"] -
  ...
  void SignIn(@required String email, @required String pass,
    ...
    Future<bool> validateData(@required Map<String, dynamic> userData) async {
      DocumentSnapshot WrongNames =
          await Firestore.instance.collection("Users").document("admin").get();
      ...
      try {
        ...
        for (int i = 0; i < wrongNames.length; i++) {
          Firestore.instance
              .collection('Commons')
              .document('Words')
              .setData({"wrongName": wrongNames[i]});
        }
      } catch (e) {
        debugPrint("===========================");
